...
async fn test_with_vault<'d, F, R>(
    docker: &'d Cli,
    execute: impl FnOnce(InterBtcParachain, VaultId, InterBtcParachain, GgxNodeContainer<'d>) -> F,
) -> R
where
    F: Future<Output = R>,
{
    ...
    let parachain_rpc = setup_provider(AccountKeyring::Bob, &container).await;
    ...
    let vault_provider = setup_provider(AccountKeyring::Charlie, &container).await;
    ...
    execute(parachain_rpc, vault_id, vault_provider, container).await
}
...
async fn test_redeem_succeeds() {
    ...
    test_with_vault(&docker, |_client, vault_id, vault_provider, container| async move {
        let relayer_provider = setup_provider(AccountKeyring::Bob, &container).await;
        let user_provider = setup_provider(AccountKeyring::Dave, &container).await;
...
async fn test_replace_succeeds() {
    ...
    test_with_vault(
        &docker,
        |_client, old_vault_id, old_vault_provider, container| async move {
            let relayer_provider = setup_provider(AccountKeyring::Bob, &container).await;
            let new_vault_provider = setup_provider(AccountKeyring::Eve, &container).await;
...
async fn test_cancellation_succeeds() {
    ...
    let docker = Cli::default();
    test_with_vault(&docker, |_, old_vault_id, old_vault_provider, container| async move {
        let root_provider = setup_provider(AccountKeyring::Alice, &container).await;
        let relayer_provider = setup_provider(AccountKeyring::Bob, &container).await;
        let new_vault_provider = setup_provider(AccountKeyring::Eve, &container).await;
        let new_vault_id = VaultId::new(
            AccountKeyring::Eve.into(),
...
async fn test_issue_overpayment_succeeds() {
    ...
    test_with_vault(
        &docker,
        |_root_provider, vault_id, vault_provider, container| async move {
            let relayer_provider = setup_provider(AccountKeyring::Bob, &container).await;
            let user_provider = setup_provider(AccountKeyring::Dave, &container).await;
...
async fn test_automatic_issue_execution_succeeds() {
    ...
    test_with_vault(
        &docker,
        |_root_provider, vault1_id, _vault1_provider, container| async move {
            let relayer_provider = setup_provider(AccountKeyring::Bob, &container).await;
            let vault1_provider = setup_provider(AccountKeyring::Charlie, &container).await;
            let vault2_provider = setup_provider(AccountKeyring::Eve, &container).await;
        ...
        test_with_vault(
            ...
            |_root_provider, vault_id, vault_provider, container| async move {
                ...
                let relayer = Runner::new(
                    ...
                    user_provider.clone(),
                    Config {
                        start_height: Some(max(1, height.saturating_sub(200))), /* important to skip the genesis
                                                                                 * block
                                                                                 * since it has nVersion < 4, so it
                                                                                 * would
                                                                                 * get rejected */
                        max_batch_size: 16,
