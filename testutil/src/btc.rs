...
impl<'d> BtcNodeContainer<'d> {
    ...
    pub fn get_p2p_port(&self) -> u16 {
        self.0.get_host_port_ipv4(18444)
    }
    ...
    pub fn api(&self) -> Client {
        let url = format!("http://{}:{}", self.get_host(), self.get_rpc_port());
        Client::new(url.as_str(), Auth::UserPass(self.get_username(), self.get_password()))
